    ...
    return [
      ...
      },
      {
        ...
        points: rows.map((row) => ({
          date: row.pk_date.toISOString().slice(0, 10),
          value:
            row.highest_statsValue !== null && row.lowest_statsValue !== null
  ...
  return [
    ...
    },
    {
      ...
      points: rows.map((row) => ({
        date: row.pk_date.toISOString().slice(0, 10),
        value:
          row.highestRespirationValue !== null && row.lowestRespirationValue !== null
            ? row.highestRespirationValue - row.lowestRespirationValue
            : null,
      })),
      smoothingWindow: 7,
      colorStart: "#65a30d",
...
function analyzeEventImpact(
  ...
  for (const event of events) {
    const startTime = new Date(`${event.startDate}T00:00:00.000Z`).getTime();
    ...
    analyses.push({
      ...
      mediumTerm,
      longTerm,
      hasConflictingSignals: meaningfulDirections.includes("higher") && meaningfulDirections.includes("lower"),
    });
  }
...
export default async function PrivateSlugPage({ params }: PrivateSlugPageProps) {
  ...
  const lowerIsBetter = metric.key === "stress" || metric.key === "respiration";
                ...
                {eventImpactAnalyses.map((analysis) => (
                  <article key={analysis.eventId} className={styles.eventImpactItem}>
                    ...
                    {analysis.shortTerm ? (
                      <>
                        <p>
                          {metric.label} became {analysis.shortTerm.direction} by {analysis.shortTerm.percentChange.toFixed(1)}%.
                        </p>
                        <p>Interpretation: {analysis.shortTerm.interpretation}</p>
                      </>
                    ) : (
                    ...
                    {analysis.mediumTerm ? (
                      <>
                        <p>
                          {metric.label} was {analysis.mediumTerm.direction} by {analysis.mediumTerm.percentChange.toFixed(1)}% compared to baseline.
                        </p>
                        <p>
                          Z-score:{" "}
                          {analysis.mediumTerm.zScore === null ? "n/a" : analysis.mediumTerm.zScore.toFixed(2)}
                        </p>
                        <p>Interpretation: {analysis.mediumTerm.interpretation}</p>
                      </>
                    ) : (
                    ...
                    {analysis.longTerm ? (
                      <>
                        <p>
                          {metric.label} is {analysis.longTerm.direction} by {analysis.longTerm.percentChange.toFixed(1)}%.
                        </p>
                        <p>Interpretation: {analysis.longTerm.interpretation}</p>
                      </>
                    ) : (
